...
jQuery.fn.pagination = function(maxentries, opts){
	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			var pagetext = '<input class="pagevalue" size="1" maxlength="8" value="'+(parseInt(current_page)+1)+'"type="text">';
            var toPage='<span class="pagejump"><a class="search" href="#">跳转</a></span>';
            $(pagetext).appendTo(panel);
            $(toPage).appendTo(panel);
            $(document).on("keyup",".pagevalue",function() {
            	$(this).val($(this).val().replace(/[^\d]/g,''));
            });
            $(".search").bind("click",function(evt){
            	//var pagevalue = $.trim($("#pagevalue").val());
            	var pagevalue = $.trim($(this).parents("div").find(".pagevalue").val());
            	if(pagevalue){
            		 var iPageNum = pagevalue -1;
                     if(iPageNum<0){
                     	alert("请输入正确的页码");
                     	}else if(iPageNum<np){
